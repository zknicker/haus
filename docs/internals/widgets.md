...
- **Where the fence may sit.** The grammar is forgiving about position and
  ...
  a line *or* glued to the end of prose — a model that ends a sentence and
  writes the tag without a line break still gets a visual — and the body ends
  at the first backtick run of a body line, whether that run stands alone or
  sits at the end of the markup, right after a closing `</script>`. Text on
  either side of the fence stays prose. Whitespace in front of the tag still
  reads as prose, a backtick in front is a longer fence or inline code, the
  info word must be exactly `visual`, a terminator must end its line or have
  whitespace after it, and a tag inside another fenced block — the
  four-backtick examples the visuals skill ships — belongs to that block. The
  skill still instructs agents to start the fence on its own line: tolerance
  in the parser, clarity in the instruction. The `artifact` fence reads
  mid-line on both ends too
  (`apps/website/src/features/servers/chat/artifact-message.tsx`), and the iOS
  parser (`apps/ios-swift/Sources/HausUI/Visuals/VisualFence.swift`) mirrors
  the web grammar case for case.
