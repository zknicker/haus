...
## Components
- **Amazon references:** Product chips reuse the inline reference shell with an 18px transparent cutout thumbnail from RankWrangler and a short name (ASIN fallback) capped at 16rem. The image fits inside its box without cropping, zoom, or background blending. Missing or failed cutouts use the product icon. Their landscape hover card uses the shared hover-card material at 384px and shows a compact text surface with a two-line title, closely spaced brand, and price, with a 72px transparent cutout floating to its right at −5°. A 1.2-second pop-in with diminishing scale and rotation overshoot settles at −5°; a delayed sparkle fades out. Both respect reduced-motion preferences. Feature bullets are omitted.
...
- **Inline references:** Use the transparent HeroUI Chip shell with inherited font size, a tight internal line box, no outer padding, one `--spacing` gap, and an 18px identity mark, reduced to 16px for the compact three-sparkle Skill mark. Use a bold label with a dotted underline in the reference's identity color; reserve the underline space inside the label so truncation cannot clip it, and scale its dot size and spacing with the surrounding type. Optically raise the fixed-size mark by 1.5px to align it with the surrounding text. The three-sparkle glyph has extra inline-end whitespace in its official viewBox, so Skill chips alone pull that edge inward by half a spacing step; Agent and Channel spacing stays unchanged. Agent labels use accent, Skill labels use the mode-aware purple `--skill-reference` product token, and Channel labels use their configured Channel color; a colorless Channel falls back to foreground ink with a translucent foreground-wash mark, never `--default` or `--muted` — the sidebar row's hover and current fills are `--default`, so an opaque box built from it vanishes into the exact states meant to emphasize the row, and a wash composites over any fill. In the light theme only, the larger sidebar Channel mark uses a slightly quieter wash than compact reference and preview marks; dark uses one shared ground and one shared fill. The Channel mark, glyph, and label resolve per theme on `channel-icon-box` and `reference-chip--channel` in `default-theme.css`, with the always-dark preview keyed into the dark branch; Skill chips resolve on `reference-chip--skill`, and the contrast preview rebinds the Skill token to its dark value. Interactive references strengthen on hover and focus by moving away from their ground: light deepens, dark brightens. Pointer-following rich previews use non-interactive Tooltips. Their bottom-left corner tracks 25px right and 25px above the pointer without a travel cap (subject to viewport collision bounds), close when the trigger is left, and do not accept pointer input. Runtime help remains an anchored interactive HoverCard for its copy control. Pointer-following updates the individual `translate` property without replacing React Aria placement transforms. The Tooltip trigger itself is middle-aligned so its wrapper cannot move a reference off the paragraph baseline. Channel previews render participant identities below the header as an overlapping 20px `EntityAvatar` stack, each mark ringed in the card's opaque ground so the stack does not smear into one shape, with any remainder as a trailing muted count. The paragraph owns leading; references should read as enhanced text, not standalone badges. Preview anatomy is under **Hover cards**.
- **Hover cards:** Every preview hover card — Agent, Channel, Skill, product, reference fallback, Trigger and Reminder marks, the session mark, and runtime help — is one material: `haus-hover-card dark` (`hausHoverCardClassName`) in `default-theme.css`. It is always-dark glass in both app themes: an eclipse ground at 88% under a 20px backdrop blur, near-opaque so text contrast never depends on what sits behind it, a 9% snow hairline inner edge, and a compact three-step inset. The `dark` class scopes HeroUI's stock dark tokens so nested Alerts, Chips, and code snippets flip with the card; product tokens live on `:root` only, so the ones cards use (Skill, reasoning, Trigger, Reminder, and session inks) are rebound on the card; nested surfaces are a faint snow wash rather than a second slab. Reference and identity cards open with the shared `ReferencePreviewHeader`: an 18px identity mark (16px for the Skill sparkle and the Trigger, Reminder, session, and warning glyphs, in their own ink), a semibold `text-sm` title, and one muted `text-xs` `·` clause carrying what that kind knows about itself — a Channel's activity, a Skill's kind, an Agent's availability, a Trigger's kind, a session's reason. Reminder cadence sits below the title to preserve its width. Supporting content is `text-xs` directly below: muted for facts and captions, foreground for the subject's own words. Optically hang the mark columns — the identity row and the participant stack — one pixel toward the block/start edges. Widths come in three measures: compact cards size to content up to 288px, the Agent card is 320px, and the product card is 384px so a listing title fits two lines. Cards are previews, not settings panels: a fact line such as `Armed · Last fired 4m ago · 12 fires` replaces labelled rows, and they carry no management or navigation links — only essential recovery controls such as runtime help's copyable sign-in command.
- **Activity history:** Turn rows open and close with one animated height transition, the
  stock 200ms one, and the chevron rotates both ways; reduced motion places them at once. A
