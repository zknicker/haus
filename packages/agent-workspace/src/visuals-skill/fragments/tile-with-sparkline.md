...
the tile's words already carry the value. The row shares one period, so each
title is the metric alone and the fence title names the period; a row that
mixes periods titles each tile `Revenue · 30d`, per the tile grammar in
components.md.
